...
export default function SchoolPage() {
  ...
  const levelOrgs = useMemo(() => {
    if (!allOrgs) return [];
    return allOrgs.filter(org => org.org_type_id === 3);
  }, [allOrgs]);
              ...
              <>
                ...
                <div className="mt-8">
                  <div className="flex gap-1 mb-4 p-1 rounded-md overflow-x-auto scrollbar-thin" style={{ background: "var(--surface-mid)" }}>
                    {["contingent", "finance", "graduates", "education"].map(tab => {
                  ...
                  <div className="mt-4 card p-6">
                    {activeFormTab === "contingent" && (
                      <ContingentForm orgId={selectedOrgId || undefined} />
                    )}
                    {activeFormTab === "finance" && (
                      <FinanceForm orgId={selectedOrgId || undefined} />
                    )}
                    {activeFormTab === "graduates" && (
                      <GraduatesForm orgId={selectedOrgId || undefined} />
                    )}
                    {activeFormTab === "education" && (
                      <EducationForm orgId={selectedOrgId || undefined} />
                    )}
                  </div>
                </div>
              </>
