...
export function AnalyticsGlobalStatsPage() {
  ...
  const { data: pingResp, loading, error } = useApi<{ result: unknown[] }>(
    "/admin/superset/dashboards"
  );
  const supersetReady = !loading && !error && pingResp !== null;
      ...
      {!loading && error && (
        <div className="card p-8 flex flex-col items-center gap-4 text-center">
          <div className="w-14 h-14 rounded-full flex items-center justify-center"
               style={{ background: "rgba(0,104,180,0.1)" }}>
            <BarChart3 className="w-7 h-7" style={{ color: "var(--fc-blue)" }} />
          </div>
          <div>
            <p className="font-display font-bold text-base mb-1" style={{ color: "var(--text-primary)" }}>
              BI-аналитика недоступна
            </p>
            <p className="text-sm mb-4" style={{ color: "var(--text-muted)" }}>
              Сервис Superset не запущен или не настроен на этом сервере.
            </p>
          </div>
